...
| Tenant invite onboarding | IDP tenant admin API | `idp`, `communication` | Tenant invite API -> communication delivery client | email delivery + outcome polling | API `ProblemDetails`, operator logs | in-progress | Delivery client and token acquisition failures now preserve structured chains. Invite creation now surfaces notification degradation in the created payload, but delivery outcome polling remains best-effort. |
